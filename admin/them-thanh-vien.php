<?php
	...
	$username= $password= $repassword= $name= $email= $birth= $sex= "";
	...
	if ($_SERVER["REQUEST_METHOD"]== "POST") {
		...
		if ($usernameErr== "" && $passwordErr== "" && $repasswordErr== "" && $nameErr== "" && $emailErr== "" && $birthErr== "" && $sexErr== "") {
			$sql= "INSERT INTO users (username, password, name, email, birth, sex)
					VALUES ('$username', md5('$password'), '$name', '$email', '$birth', '$sex')";
		...
		<div class="row">
			<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post">
				...
				<table class="table">
					...
					<tr>
						<td></td>
						<td><input type="submit" name="btn_them" value="THÊM MỚI"></td>
					</tr>
				</table>
			</form>
